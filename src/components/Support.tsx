...
export function SupportBlock({
  ...
  const getPayload = useCallback(() => {
    ...
    if (config && amount < config.min) {
      setError(
        `The smallest amount PayPal will process here is ${config.symbol}${formatMajor(
          config.min,
          config.decimals
        )}.`
      );
  ...
  if (status === "done") {
    return (
      <section className="border border-[color:var(--color-accent)]/40 bg-[color:var(--color-bg-elevated)]/60 p-8 md:p-12 text-center">
  ...
  return (
    <section className={`border border-[color:var(--color-rule)] bg-[color:var(--color-bg-elevated)]/40 ${compact ? "p-6 md:p-8" : "p-6 md:p-12"}`}>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14">
        ...
        <div className="lg:col-span-6">
          {loadFailed && (
            <div className="border border-[color:var(--color-rule)] p-6 text-sm text-[color:var(--color-muted)]">
          ...
          {config?.configured && (
            <>
              <fieldset>
                ...
                {customTooSmall && (
                  <p className="mt-2 text-xs text-[color:var(--color-faint)]">
